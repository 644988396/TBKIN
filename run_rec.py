            ...
            for step, data in enumerate(loader):
                ...
                for step_ix in range(pred_refs_rank.shape[0]):
                    ...
                    overlaps = bbox_overlaps(
                        np.ascontiguousarray(cliped_bbox_reg_ix[arg_pred_refs_rank[step_ix]][np.newaxis, :], dtype=np.float64),
                        np.ascontiguousarray(refs_gt_boxes[step_ix], dtype=np.float64))[:, 0]
                    conut += 1
